...
{
    public class Magnet : MonoBehaviour, ICollectable
    {
        ...
        private void Collect(Transform collector)
        {
            ...
            if (GameStatus.CurrentState != GameState.Playing)
                return;
            List<Transform> activeCollectables = PoolBoss.CategoryActiveItems("Collectables");
            foreach (var activeCollectable in activeCollectables)
            {
                IAttractable attractable = activeCollectable.GetComponent<IAttractable>();
                if (attractable != null)
                    attractable.OnAttracted(collector);
            }
        }
